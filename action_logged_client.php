<?php
    ...
    if(isset($_POST['action'])){
        ...
        if(!empty($_POST['startDate']) && !empty($_POST['endDate'])){
            ...
            $sql .= "AND room.roomID NOT IN (SELECT reservation.fk_roomID FROM reservation WHERE ('".$start."' <= reservation.reservationEnd 
                     AND '".$end."' >= reservation.reservationStart));";
        }
    }
...
if($result->num_rows > 0){
    while($row=$result->fetch_assoc()){
        ...
        $output .= "<form class='select_room_form_data'>";
        ...
        $output .= "</div>";
        $output .= "</div>";
        $output .= "<button type='button' name='".$row['capacity']."' class='orderRoom w3-btn w3-blue w3-medium btn-block' value='".$row['label']."' style='margin-top:2px; border-radius: 5px 5px 5px 5px'>Zamów</button>";
        $output .= "</form>";
        $output .= "</div>";
        $output .= "</div>";
            }
